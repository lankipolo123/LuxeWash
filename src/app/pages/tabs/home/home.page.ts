...
export class HomePage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Initialize the filtered products with shuffled products
  ...
  shuffleProducts() {
    ...
    this.filteredProducts = [...this.products]; // Set filteredProducts to shuffled products
  }

  selectCategory(category: Category) {
    // Filter the products based on the selected category without shuffling
  ...
  filterProducts() {
    ...
    this.filteredProducts = this.products.filter(product => product.name.toLowerCase().includes(term));
  }
  ...
  async addToCart(event: Event, product: Product) {
   ...
   await this.presentToast(`${product.name} added to cart!`);
}
  ...
  navigateToProductDetails(product: Product) {
    ...
    this.router.navigate(['tabs/product-detail', product.id]);
  }
}
